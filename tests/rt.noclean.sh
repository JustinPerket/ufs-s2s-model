...
#  ACCNR= # detected in detect_machine.sh
  ACCNR=gfdlhires
  PARTITION=orion
  # JP changed because can't create stmp folder
...
while read -r line; do
  ...
  if [[ $line == COMPILE* ]] ; then
      ...
      if [[ $ROCOTO == true ]]; then
        ...
        ./compile.sh $PATHTR/FV3 $MACHINE_ID "${NEMS_VER}" $COMPILE_NR clean_before=NO clean_after=NO > ${LOG_DIR}/compile_${COMPILE_NR}.log 2>&1
